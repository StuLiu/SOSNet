...
class Metrics:
    ...
    def compute_iou(self) -> Tuple[Tensor, Tensor]:
        ...
        if self.ignore_label < self.num_classes:
            ious[self.ignore_label] = torch.tensor(float('nan'))
    ...
    def compute_f1(self) -> Tuple[Tensor, Tensor]:
        ...
        if self.ignore_label < self.num_classes:
            f1[self.ignore_label] = torch.tensor(float('nan'))
    ...
    def compute_pixel_acc(self) -> Tuple[Tensor, Tensor]:
        ...
        if self.ignore_label < self.num_classes:
            acc[self.ignore_label] = torch.tensor(float('nan'))
