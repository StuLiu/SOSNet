...
class Conv2d_BN(nn.Sequential):
    def __init__(self, a, b, ks=1, stride=1, pad=0, dilation=1,
        ...
        self.add_module('c', nn.Conv2d(
            a, b, (ks, ks), (stride, stride), (pad, pad), (dilation, dilation), groups, bias=False))

        if norm_cfg is not None:
            bn = norm_cfg(b, affine=True)
...
class ConvModule(nn.Module):
    ...
    # ConvModule(inp, oup, kernel_size=1, norm_cfg=self.norm_cfg, act_cfg=None)
    def __init__(self, inp, oup, kernel_size, stride=1, pad=0, dilation=1, groups=1,
                 norm_cfg=nn.BatchNorm2d,
                 act_cfg=nn.ReLU,
                 bias='auto'):
        super().__init__()
        ...
        self.conv = nn.Conv2d(inp, oup,
                              ...
                              groups=groups,
                              bias=bias)
        assert norm_cfg is None or norm_cfg in [nn.BatchNorm2d]
        assert act_cfg is None or act_cfg in [nn.ReLU, nn.LeakyReLU, nn.Tanh, nn.ReLU6, h_sigmoid]

        self.is_norm = norm_cfg is not None
        self.is_act = act_cfg is not None
        if self.is_norm:
            self.norm = norm_cfg(oup, affine=True)
        if self.is_act:
            self.activate = act_cfg(inplace=True)
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.ReLU, drop=0.,
        ...
        self.dwconv = nn.Conv2d(hidden_features, hidden_features, (3, 3), (1, 1), (1, 1),
...
class Block(nn.Module):
    ...
    def __init__(self, dim, key_dim, num_heads, mlp_ratio=4., attn_ratio=2., drop=0.,
                 drop_path=0., act_layer=nn.ReLU, norm_cfg=nn.BatchNorm2d):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
...
class InjectionMultiSum(nn.Module):
    def __init__(
        ...
        self.local_embedding = ConvModule(inp, oup, kernel_size=1, norm_cfg=norm_cfg, act_cfg=activations)
        self.global_embedding = ConvModule(inp, oup, kernel_size=1, norm_cfg=norm_cfg, act_cfg=activations)
        self.global_act = ConvModule(inp, oup, kernel_size=1, norm_cfg=norm_cfg, act_cfg=activations)
        self.act = h_sigmoid()
...
class InjectionMultiSumCBR(nn.Module):
    def __init__(
        ...
        self.local_embedding = ConvModule(inp, oup, kernel_size=1, norm_cfg=norm_cfg)
        # global_embedding: conv-bn-relu
        self.global_embedding = ConvModule(inp, oup, kernel_size=1, norm_cfg=norm_cfg)
        # global_act: conv
        self.global_act = ConvModule(inp, oup, kernel_size=1, norm_cfg=None, act_cfg=activations)
        self.act = h_sigmoid()
...
class FuseBlockSum(nn.Module):
    def __init__(
        ...
        self.fuse1 = ConvModule(inp, oup, kernel_size=1, norm_cfg=norm_cfg, act_cfg=activations)
        self.fuse2 = ConvModule(inp, oup, kernel_size=1, norm_cfg=norm_cfg, act_cfg=activations)
...
class TokenPyramidTransformer(nn.Module):
    ...
    def load_pretrained(self, pretrained, strict=False):
...
if __name__ == "__main__":
    ...
    _model = TokenPyramidTransformer(**topformer_cfgs['B'])
    _model.load_pretrained('../../../checkpoints/backbones/topformer/topformer-B-224-75.3.pth')
    _model = _model.cuda()
    _out = _model(_x)
    for _out_i in _out:
        print(_out_i.size())
    ...
    init_logger()
    model_summary(_model, (1, 3, 224, 224))
