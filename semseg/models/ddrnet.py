...
class BasicBlock(nn.Module):
    """
    Basic block for DDRNet.
    Res(Conv->Bn->Relu->Conv->Bn[->Relu])
    Args:
        c1: in channels: int,
        c2: out channels: int,
        s: stride: int,
        downsample: downsample input tensor for residual op: torch.nn.Module
        no_relu: activate the residual output if true: bool
    """
    expansion = 1
...
class Bottleneck(nn.Module):
    ...
    Res(Conv->Bn->Relu->Conv->Bn->Relu->Conv->Bn[->Relu])
    Args:
        c1: input channel count: int
        c2: output channel count: int
        s: stride: int
        downsample: downsample module: torch.nn.Module
        no_relu: whether using relu in top of the bottleneck: bool
    """
    expansion = 2
...
class Stem(nn.Sequential):
    """
    The stem(first and second conv layers of DDRNet) of backbone
    Args:
        c1: input channel count: int
        c2: output channel count: int
    """
...
class DAPPM(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        outs.append(self.process1(
            (F.interpolate(self.scale1(x), size=x.shape[-2:], mode='bilinear', align_corners=True) + outs[-1])))
        outs.append(self.process2(
            (F.interpolate(self.scale2(x), size=x.shape[-2:], mode='bilinear', align_corners=True) + outs[-1])))
        outs.append(self.process3(
            (F.interpolate(self.scale3(x), size=x.shape[-2:], mode='bilinear', align_corners=True) + outs[-1])))
        outs.append(self.process4(
            (F.interpolate(self.scale4(x), size=x.shape[-2:], mode='bilinear', align_corners=True) + outs[-1])))
...
if __name__ == '__main__':
    ...
    model_summary(_model, (1, 3, 1024, 512))
