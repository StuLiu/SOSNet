...
class BiSeNetv2(BaseModel):
    ...
    def __init__(self, backbone: str = None, num_classes: int = 19) -> None:
        ...
        self.head_top = UPerHead(in_channels=[16, 32, 64, 128],
                                 channel=32,
                                 num_classes=2,
                                 scales=(1, 2, 3, 6))
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            logits_top = self.head_top([aux2, aux3, aux4, aux5])
            logits_top = F.interpolate(logits_top, x.shape[-2:], mode='bilinear', align_corners=True)
            return output, logits_top, None
        return output


if __name__ == '__main__':
    model = BiSeNetv2().cuda()
    model.eval()
